...
【重要】modify_sprite / remove_sprite の target には、必ず下記の「現在のスプライト一覧」に
実在する名前だけを指定してください。ユーザーの指示文中に「参考情報」として他のプロジェクトの
スプライト名（例: 参考ファイルの疑似コードに出てくる名前）が含まれていても、それらは編集対象の
プロジェクトには存在しません。「現在のスプライト一覧」に無い名前を挙げたい場合は、それは新規
